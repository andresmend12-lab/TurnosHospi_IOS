...
class PlantManager: ObservableObject {
    ...
    // Buscar planta por ID y Password
    ...
    // Unirse a la planta seleccionada
    func joinPlant(plant: HospitalPlant, selectedStaff: PlantStaff) {
        ...
        // 1. Guardar en plants -> ID -> userPlants -> UID
        ref.child("plants").child(plant.id).child("userPlants").child(user.uid).setValue(userPlantData) { error, _ in
            if let error = error {
                ...
                // 2. ACTUALIZAR PERFIL DE USUARIO con el ID de la planta
                // Esto es vital para que la app sepa que ya tienes planta
