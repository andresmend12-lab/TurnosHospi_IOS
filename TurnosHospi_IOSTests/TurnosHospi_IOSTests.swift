...
struct TurnosHospi_IOSApp: App {
    ...
    // 3. Crear las instancias únicas de nuestros servicios (Single source of truth)
    @StateObject var authService = AuthService()
    @StateObject var shiftRepository = ShiftRepository()
    
    var body: some Scene {
        WindowGroup {
            // 4. Llamar a la vista raíz (que crearemos a continuación)
            // Inyectamos los servicios usando .environmentObject
            RootView()
